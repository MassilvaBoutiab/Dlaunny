...
void Canvas::addTriangle(int id0, int id1, int id2, const QColor &color, int id)
{
    triangles.push_back(Triangle(id, &vertices[id0], &vertices[id1], &vertices[id2], color));
}
    ...
    for (auto &tri : triangles) {
        if (tri.isHighlighted()) {
            if (tri.isTriangleFlippable()) {
                tri.getFlipNeighbhoor()->ptr[0] = tri.neighbhorUniquePoint;
                tri.getFlipNeighbhoor()->ptr[1] = tri.getFlipNeighbhoor()->neighbhorUniquePoint;
                tri.getFlipNeighbhoor()->ptr[2] = tri.neighbhoringLine[1];

                tri.ptr[0] = tri.neighbhorUniquePoint;
                tri.ptr[1] = tri.getFlipNeighbhoor()->neighbhorUniquePoint;
                tri.ptr[2] = tri.neighbhoringLine[0];

                tri.neighbhoringLine[0] = tri.getFlipNeighbhoor()->neighbhorUniquePoint;
                tri.neighbhoringLine[1] = tri.neighbhorUniquePoint;

                tri.getFlipNeighbhoor()->computeCircle();
...
Vector2D *Canvas::findNeighbhoor(Vector2D first, Vector2D second, Vector2D third)
{
    for (auto othertri : triangles) {
        ...
        } else if (othertri.ptr[0] == first && othertri.ptr[2] == second
                   && !(othertri.ptr[1] == third)) {
            return othertri.ptr[1];
        } else if (othertri.ptr[1] == first && othertri.ptr[0] == second
                   && !(othertri.ptr[2] == third)) {
            return othertri.ptr[2];
        } else if (othertri.ptr[2] == first && othertri.ptr[0] == second
                   && !(othertri.ptr[1] == third)) {
            return othertri.ptr[1];
        } else if (othertri.ptr[2] == first && othertri.ptr[1] == second
                   && !(othertri.ptr[0] == third)) {
            return othertri.ptr[0];
        } else if (othertri.ptr[1] == first && othertri.ptr[2] == second
                   && !(othertri.ptr[0] == third)) {
            return othertri.ptr[0];
        }
    }
...
{
    for (Triangle &othertri : triangles) {
        if (othertri.ptr[0] == first && othertri.ptr[1] == second && othertri.ptr[2] == third) {
            return &othertri;
        } else if (othertri.ptr[0] == first && othertri.ptr[2] == second
                   && othertri.ptr[1] == third) {
...
void Canvas::loadMesh(const QString &title)
{
    ...
    if (file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        ...
        for (auto &&v : JSONtriangles) {
            ...
            if (tri.size() == 3) {
                addTriangle(tri[0].toInt(), tri[1].toInt(), tri[2].toInt(), QColor(color), i++);
            }
        }
    }
    reScale();
